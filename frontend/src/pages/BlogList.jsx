      ...
      <section className="pt-36 pb-20">
        <div className="gma-container">
          <div className="max-w-3xl mb-14">
            ...
            <h1 className="gma-h1">{t('blog.title')}</h1>
          </div>

          {loading ? (
            <div className="py-20 text-center text-[hsl(var(--muted-foreground))]">{t('blog.loading')}</div>
          ) : posts.length === 0 ? (
            <div className="py-20 text-center text-[hsl(var(--muted-foreground))]">{t('blog.noPosts')}</div>
          ) : (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {posts.map((p, i) => (
                <motion.article
                  ...
                  data-testid={`bloglist-card-${i}`}
                >
                  <Link to={`/blog/${p.slug}`} className="block">
                    <div className="aspect-[16/10] overflow-hidden">
                      <img src={p.image} alt={lang === 'en' ? p.title_en : p.title_id} loading="lazy" className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
                    </div>
                    <div className="p-6">
